...
export default function BaseNode({ data, selected }: NodeProps) {
	...
	const hasBrokenReferences = functions.some((func) => func.next_node_id && !nodeIds.has(func.next_node_id));
	...
	return (
		...
		>
			...
			<div className="flex items-center gap-1.5">
				{isInitialNode && <Play className="h-3 w-3 text-neutral-400 dark:text-neutral-500 shrink-0" />}
			...
			{!isEndNode && <Handle type="source" position={Position.Bottom} className="bg-neutral-400!" />}
		</div>
	);
}

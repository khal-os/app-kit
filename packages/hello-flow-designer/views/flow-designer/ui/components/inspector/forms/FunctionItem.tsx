...
export const FunctionItem = React.forwardRef<HTMLDivElement, FunctionItemProps>(
	(
		{ func, onChange, onRemove, availableNodeIds, functionIndex, isSelected, selectedConditionIndex, currentNodeId },
		ref
	) => {
		// Check if next_node_id is invalid (pointing to a deleted node)
		const hasInvalidNextNode = func.next_node_id !== undefined && !availableNodeIds.includes(func.next_node_id);
					...
					<div className="p-4 space-y-4">
						...
						<div className="pt-3 border-t border-neutral-200 dark:border-neutral-700">
							...
							{propertyEntries.length === 0 ? (
								<div className="text-xs opacity-40 italic py-2">No properties. Click "Add Property" to create one.</div>
							) : (
								<div className="space-y-2">
						...
						<div className="pt-3 border-t border-neutral-200 dark:border-neutral-700">
							...
							{availableNodeIds.length > 0 ? (
								<Select
									value={func.decision ? func.decision.default_next_node_id : (func.next_node_id ?? undefined)}
									onValueChange={(v) => {
										if (func.decision) {
											onChange({
												decision: {

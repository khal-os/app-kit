...
import type { FlowFunctionJson } from '../schema/flow.schema';
import type { FlowEdge, FlowNode, ReactFlowInstance } from '../types/flowTypes';
...
interface EditorState {
	...
	selectNode: (nodeId: string | null, functionIndex?: number | null, conditionIndex?: number | null) => void;
	...
	selectFunction: (nodeId: string, functionIndex: number, nodes: FlowNode[], conditionIndex?: number | null) => void;
...
export const useEditorStore = create<EditorState>((set, get) => {
	return {
		...
		selectNodeFromEdge: (edge, nodes) => {
			...
			const targetDecisionNode = nodes.find((n: FlowNode) => n.id === edge.target && n.type === 'decision');
			...
			if (targetDecisionNode) {
				...
				if (match) {
					...
					if (sourceNode) {
						...
						const functionIndex = functions.findIndex((f) => f.name === functionName && f.decision !== undefined);
			...
			const decisionNode = nodes.find((n: FlowNode) => n.id === edge.source && n.type === 'decision');
						...
						if (functionIndex >= 0) {
							...
							if (edge.label === 'default') {
								...
								if (condMatch) {
									...
									const condIndex = func.decision?.conditions.findIndex((c) => c.next_node_id === edge.target) ?? -1;
									if (condIndex >= 0) {

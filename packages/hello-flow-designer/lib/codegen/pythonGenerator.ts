import type { FlowFunctionJson, FlowJson, FlowNodeJson, MessageJson } from '../schema/flow.schema';
...
function generateTypeDefinition(func: FlowFunctionJson): string {
	...
	const fields = Object.entries(props).map(([key, prop]) => {
		const pyType =
			prop.type === 'integer' ? 'int' : prop.type === 'number' ? 'float' : prop.type === 'boolean' ? 'bool' : 'str';
...
function formatMessage(msg: MessageJson): string {
	...
	const contentStr = hasNewlines ? `"""${msg.content.replace(/"""/g, '\\"\\"\\"')}"""` : JSON.stringify(msg.content);
	return `            {\n                "role": "${msg.role}",\n                "content": ${contentStr}\n            }`;
}
							...
							prop.type === 'integer'
								...
								: prop.type === 'number'
									...
									: prop.type === 'boolean'
										...
										: 'str';
						const defaultValue =
							prop.type === 'integer' || prop.type === 'number' ? '0' : prop.type === 'boolean' ? 'False' : '""';
...
`;
	...
	if (contextStrategy && contextStrategy.strategy !== 'APPEND') {
		if (contextStrategy.strategy === 'RESET_WITH_SUMMARY') {
			const summaryPrompt = contextStrategy.summary_prompt ? escapePythonString(contextStrategy.summary_prompt) : '';
...
export function generatePythonCode(flow: FlowJson): string {
	...
	for (const node of nodes) {
		...
		for (const td of funcGen.typeDefs) allTypeDefs.add(td);
	...
	const globalFuncRefs = globalFuncs.length > 0 ? globalFuncs.map((f) => `${f.name}_func`).join(', ') : '';

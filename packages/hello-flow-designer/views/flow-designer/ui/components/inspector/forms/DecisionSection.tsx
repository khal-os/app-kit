...
export function DecisionSection({
	...
	}, [needsDecisionExpansion, isSelected, selectedConditionIndex, scrollTarget, currentNodeId, functionIndex]);
		...
		<div className="pt-3 border-t border-neutral-200 dark:border-neutral-700">
			<div className="mb-3 flex items-center justify-between">
				<button
					...
					className="flex items-center gap-1.5 text-xs font-medium opacity-80 hover:opacity-100 transition-opacity"
				>
					{showDecision ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
					Decision
				</button>
				{func.decision && (
					<Button
							...
							<TooltipProvider>
								<Tooltip>
									...
									<TooltipContent className="max-w-xs">
										<p>
											Enter Python code that sets the <code className="font-mono">result</code> variable. For example:{' '}
								...
								{func.decision.conditions.map((condition, condIndex) => {
									...
									return (
										<ConditionItem
											...
											onRemove={() => {
												const newConditions = func.decision!.conditions.filter((_, i) => i !== condIndex);
												onChange({
													decision: {
														...func.decision!,
														conditions: newConditions,

 ...
 */
export function updateNodeData(nodes: FlowNode[], nodeId: string, updates: Partial<FlowNodeData>): FlowNode[] {
	return nodes.map((n) => {
		if (n.id === nodeId) {
 ...
 */
export function clearFunctionConnection(nodes: FlowNode[], nodeId: string, functionIndex: number): FlowNode[] {
 ...
 */
export function updateFunctionReferences(nodes: FlowNode[], oldId: string, newId: string): FlowNode[] {
	return nodes.map((node) => {
		const nodeData = node.data as FlowNodeData;
		const functions = (nodeData?.functions || []) as FlowFunctionJson[];

...
import { saveFlow } from '../../../lib/api/flowApi';
...
import { handleDecisionNodeConnection, handleRegularConnection } from '../../../lib/utils/connectionHandlers';
...
import { clearFunctionConnection, updateFunctionReferences, updateNodeData } from '../../../lib/utils/nodeUpdates';
...
export default function FlowDesignerApp() {
	const initial = useInitialGraph();
	const [nodes, setNodes, onNodesChangeBase] = useNodesState(initial.nodes);
	const [edges, setEdges, onEdgesChange] = useEdgesState<Edge>(initial.edges);

	// Flow metadata for save/load
	const [flowMeta, setFlowMeta] = useState<{
		slug: string | null;
		name: string;
		dirty: boolean;
	}>({ slug: null, name: 'Untitled', dirty: false });

	// Mark dirty on changes
	useEffect(() => {
		setFlowMeta((prev) => (prev.dirty ? prev : { ...prev, dirty: true }));
	}, [nodes, edges]);
			...
			if (positionChanges.length > 0) {
				setNodes((currentNodes) => {
					...
					positionChanges.forEach((change) => {
						...
						const sourceNodeIndex = updatedNodes.findIndex((n) => n.id === decisionInfo.sourceNodeId);
	...
	const validateFunctionIndexAfterUpdate = useEditorStore((state) => state.validateFunctionIndexAfterUpdate);
	...
	// Keep selected node visually selected
	...
	// Push to undo history on changes
	useEffect(() => {
		...
		const handler = (e: KeyboardEvent) => {
			...
			if (modKey && e.key === 's') {
				...
				if (flowMeta.slug) {
					...
					saveFlow(flowMeta.slug, flowMeta.name, undefined, json)
						.then(() => {
							setFlowMeta((prev) => ({ ...prev, dirty: false }));
							showToast('Flow saved', 'success');
						})
						.catch(() => {
							showToast('Failed to save. Changes saved locally.', 'error');
						});
				}
			}
			if (modKey && e.key === 'e') {
				e.preventDefault();
				useEditorStore.getState().setShowJson(!useEditorStore.getState().showJson);
			}
		};
		window.addEventListener('keydown', handler);
		return () => window.removeEventListener('keydown', handler);
	}, [flowMeta, nodes, edges]);
	...
	return (
		<div className="h-full w-full flex overflow-hidden">
			<div
				...
				style={{
					height: `calc(100% - ${showJson ? jsonEditorHeight : 0}px)`,
				}}
			>
				<div
			...
			<Toolbar
				...
				onNewFlow={() => {
					...
					clearSelection();
					setFlowMeta({ slug: null, name: 'Untitled', dirty: false });
					setTimeout(() => {
						if (rfInstance) {
			...
			>
				<ReactFlow
					...
					onDrop={(e) => {
						...
						if (type === 'initial') {
							const hasInitialNode = nodes.some((n) => deriveNodeType(n.data, n.type as string) === 'initial');
							if (hasInitialNode) return;
						}
				...
				<NodeContextMenu
					...
					isDecisionNode={
						contextMenuNodeId ? nodes.find((n) => n.id === contextMenuNodeId)?.type === 'decision' : false
					}
				/>
			</div>
			<div
								...
								if (kind === 'edge') {
									...
									if (functionIndex >= 0) {
										...
										setNodes((nds) => {
											...
											const newFunctions = (updatedNode?.data?.functions ?? []) as FlowFunctionJson[];
											validateFunctionIndexAfterUpdate(edge.source, previousFunctions, newFunctions);
											return updatedNodes;
										});
